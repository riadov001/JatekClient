...
export default function CartScreen() {
  ...
  const { data: restaurantData, isLoading: restaurantStatusLoading } = useGetRestaurant(restaurantId ?? 0, {
    query: { queryKey: getGetRestaurantQueryKey(restaurantId ?? 0), enabled: !!restaurantId },
  });
  const isRestaurantClosed = !!restaurantId && !!restaurantData && (restaurantData as { isOpen?: boolean | null }).isOpen === false;
  ...
  const orderDisabled = createOrder.isPending || (!!address && !selectedAddressInZone) || isRestaurantClosed || (!!restaurantId && restaurantStatusLoading);
      ...
      <View style={[styles.footer, { paddingBottom: insets.bottom + (Platform.OS === "web" ? 34 : 16), borderTopColor: colors.border }]}>
        {isRestaurantClosed && (
          <View style={styles.zoneBlocker}>
            ...
            <Text style={styles.zoneBlockerText}>{t("cart_restaurant_closed_warning")}</Text>
          </View>
        )}
        {!!address && !selectedAddressInZone && !isRestaurantClosed && (
          <View style={styles.zoneBlocker}>
        ...
        <ComicsCheckoutButton
          ...
          label={isRestaurantClosed ? t("cart_restaurant_closed") : !!address && !selectedAddressInZone ? t("cart_address_out_zone_btn") : t("cart_place_order")}
