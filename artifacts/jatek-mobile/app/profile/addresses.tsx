...
export default function AddressesScreen() {
  ...
  const load = useCallback(async () => {
    try { setItems(await listAddresses()); }
      ...
      // Auth errors are handled by the dedicated "Connexion requise" modal below — don't show the error popup for them.
      if (/auth/i.test(msg) || /401/.test(msg)) return;
      friendly.show({
        tone: "error",
  ...
  const onMapPick = useCallback(async (c: { latitude: number; longitude: number }) => {
    setCoords(c);
    ...
    >
      ...
      {authLoading || loading ? (
        ...
        <View style={styles.center}>
          ...
          <Text style={[styles.authTitle, { color: colors.heading }]}>Connexion requise</Text>
          <Text style={[styles.authMsg, { color: colors.mutedForeground }]}>
            Vous devez vous connecter pour accéder à vos adresses.
          </Text>
          <View style={{ flexDirection: "row", gap: 12, marginTop: 20, width: "100%", maxWidth: 360 }}>
            <TouchableOpacity
              onPress={() => router.canGoBack() ? router.back() : router.replace("/(tabs)")}
              style={[styles.btn, { backgroundColor: colors.muted, flex: 1 }]}
            >
              <Text style={[styles.btnText, { color: colors.heading }]}>Retour</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => router.push("/(auth)/login" as any)}
              style={[styles.btn, { backgroundColor: colors.primary, flex: 1 }]}
            >
              <Text style={[styles.btnText, { color: "#fff" }]}>Se connecter</Text>
            </TouchableOpacity>
          </View>
        </View>
      ) : (
...
const styles = StyleSheet.create({
  ...
  authIconWrap: { width: 72, height: 72, borderRadius: 36, alignItems: "center", justifyContent: "center", marginBottom: 4 },
  authTitle: { fontSize: 20, fontFamily: "Inter_700Bold", marginTop: 8, textAlign: "center" },
  authMsg: { fontSize: 14, fontFamily: "Inter_400Regular", textAlign: "center", lineHeight: 20, marginTop: 4, paddingHorizontal: 12 },
